...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.productReachProfile.findUnique({
    where: { productId: id },
  ...
  if (body.status === "DECLARED" || body.status === "VERIFIED") {
    const trackFields = ["containsSvhc", "declarationStatus", "supplierDeclarationRecv"] as const;
    ...
    await logCorrection(id, "reach", changed, existing?.product.productName, existing?.product.category ?? undefined);
  }
