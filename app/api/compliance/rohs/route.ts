...
export async function GET(request: NextRequest) {
  ...
  const [profiles, total] = await Promise.all([
    prisma.productRohsProfile.findMany({
      where,
    ...
    prisma.productRohsProfile.count({ where }),
  ]);
...
export async function POST(request: NextRequest) {
  ...
  const products = await prisma.product.findMany({
    where: { OR: [{ rohsProfile: { status: "UNKNOWN" } }, { rohsProfile: null }] },
    select: { id: true },
    take: 50,
  });
  ...
  for (const { id } of products) {
    try { await estimateRohs(id, noAi); await computeComplianceScore(id); updated++; } catch { errors++; }
  }
