...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const profile = await prisma.productPopProfile.findUnique({
    where: { productId: id },
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const existing = await prisma.productPopProfile.findUnique({
    where: { productId: id },
  ...
  const profile = await prisma.productPopProfile.upsert({
    where: { productId: id },
  ...
  if (body.status === "DECLARED" || body.status === "VERIFIED") {
    const trackFields = ["containsPops", "isCompliant"] as const;
    ...
    await logCorrection(id, "pop", changed, existing?.product.productName, existing?.product.category ?? undefined);
  }
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  await estimatePop(id);
  await computeComplianceScore(id);
  const profile = await prisma.productPopProfile.findUnique({ where: { productId: id } });
  return NextResponse.json(profile);
}
