...
interface PopProfile {
  productId: string;
  status: string;
  containsPops: boolean | null;
...
export default function PopPage() {
  const [data, setData] = useState<{ profiles: PopProfile[]; total: number; page: number; pageCount: number } | null>(null);
  ...
  const load = useCallback(() => {
    ...
    fetch(`/api/compliance/pop?${p}`).then((r) => r.json()).then(setData).finally(() => setLoading(false));
  }, [search, status, page]);
  ...
  async function bulkClassify() {
    setClassifying(true);
    await fetch("/api/compliance/pop", { method: "POST" });
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">POP — Persistente organische Schadstoffe</h1>
          <p className="text-sm text-gray-500">Persistent Organic Pollutants — {data?.total ?? 0} Profile</p>
        </div>
        <button onClick={bulkClassify} disabled={classifying}
          className="px-3 py-1.5 text-sm bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50">
          {classifying ? "Klassifiziere…" : "Bulk-Klassifizierung"}
        </button>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-600">POP-Risiko</th>
                <th className="px-4 py-2.5 text-left text-xs font-semibold text-gray-600">Konform</th>
                <th className="px-4 py-2.5 text-right text-xs font-semibold text-gray-600">Methode</th>
              ...
              {(data?.profiles ?? []).map((p) => (
                <tr key={p.productId} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600 text-xs">
                    {p.containsPops === true ? "POP-Verdacht" : p.containsPops === false ? "Kein POP" : "Unbekannt"}
                  </td>
                  <td className="px-4 py-3 text-gray-600 text-xs">
                    {p.isCompliant === true ? "Ja" : p.isCompliant === false ? "Nein" : "—"}
                  </td>
                  <td className="px-4 py-3 text-right text-gray-400 text-xs">{p.estimationMethod ?? "—"}</td>
